...
void muonspectrometer_mtc(){
 ...
 TString simpath_mu("/eos/user/a/aiuliano/public/sims_FairShip/sim_nutaudet/2026_03_24_numu_CCDIS_SiTarget/");
 ...
 TTreeReaderArray<MTCDetPoint> mtcpoints(reader,"MTCDetPoint");
 ...
 for (const MTCDetPoint& mtcpoint: mtcpoints){
  int nstation = (mtcpoint.GetDetectorID() - 1e+8)/1e+6; // 1e8 + layer_id * 1e6 + fibre_local_id;

  if (mtcpoint.GetTrackID()==1 && !InSpectrometer[nstation]){ //we fill the arrays with the muon neutrino positions
  
   XSpectrometer[nstation] = mtcpoint.GetX();
   YSpectrometer[nstation] = mtcpoint.GetY();
   ZSpectrometer[nstation] = mtcpoint.GetZ();
 ...
 for (int istation = 0; istation < nstations; istation++){
   cout<<"Total acceptance station "<<istation<<":"<<Inacceptance_weight[istation]/totalweight<<endl;
}
