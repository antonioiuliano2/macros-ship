...
//  for (Int_t i=eInBTArrayN-1; i==eInBTArrayN-1; --i) {
        ...
        //TEST workaround for positioning of injector when many segments are in the first plate
        eInBT_epsilonZ = 1e-3;
        InBT->SetZ(InBT->Z() - eInBT_epsilonZ); 
...
//       seg=(EdbSegP*)segarray->At(ii); // works when TObjArray
            ...
            shower_zb[ii]=seg->Z();
            if (ii==0) shower_zb[ii]+=eInBT_epsilonZ; //recover correction applied before
            ...
            if (seg->MCEvt()>0) {
                ...
                shower_ntrace4simub[ii]=seg->Track(); //used here to test injector position fix
            }
            ...
            // InBT:
            if (ii==0) {
                shower_deltathetab[0]=eAlgoParameterConnectionDT+0.350;
                shower_deltarb[0]=eAlgoParameterConnectionDR+50.;
                shower_nfilmb[0]=1;
            }
            // All other BTs:
            if (ii>0) {
                ...
                for (int jj=0; jj<shower_sizeb; jj++)  {
                    ...
                    if (test_shower_deltar<eAlgoParameterConnectionDR && test_shower_deltathetab<eAlgoParameterConnectionDT ) {
