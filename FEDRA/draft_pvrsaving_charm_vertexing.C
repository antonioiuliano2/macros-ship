...
{
  //trvol(dset);
  trvol(dset,"nseg>1 && TMath::Abs(s.eY-70000)<5000 && TMath::Abs(s.eX-20000)<5000");   
...
void do_vertex()
{
  ...
  TFile *fvtx = new TFile("vertices_uppertracks.root","RECREATE"); //OUTPUT FILE NAME
  ...
  //trying to save the edbpvrec, copying track file and reconstructed vertices
  EdbPVRec *mygAli = dproc->PVR();
  mygAli->eVTX = varr;
  mygAli->eTracks = gAli->eTracks;
  mygAli->Write(); //testing to save PVRec object
  //mygEVR->Write();
