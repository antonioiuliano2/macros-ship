...
void init( const char *def, int iopt,  const char *rcut)
{
  ...
  gAli = dproc->PVR();
  //adding missing pattern with pid 18

  EdbPattern *pat = new EdbPattern( 0., 0., -23298.03 );  
  pat->SetID(18);
  //pat->SetScanID(s1->ScanID());
  gAli->AddPatternAt(pat,18);
...
void do_vertex()
{
  ...
  EdbDataProc::MakeVertexTree(*(gEVR->eVTX),"vertextree_fourthquarter.root");
}
