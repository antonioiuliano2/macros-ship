...
void prepareTMVA_evaluation_tree(){
  TFile *f = TFile::Open("vtx_first_quarter_16_04_19.root");
  //TFile *f = TFile::Open("MC_vtx_for_training.root");
  TTree *vtx = dynamic_cast<TTree*>(f->Get("vtx"));
  //TTree *ana = dynamic_cast<TTree*>(f->Get("ana"));  // solo MC

  Char_t vtx_good;
  Int_t vID, manualcheck, ntracks, mp_pdgID, mp_motherID;
  Float_t probability, maximpactparameter, meanTX, meanTY,meannseg, vtx_max_aperture;
  Float_t maxIP, meanIP, vtx_node, vtx_holes, vtx_gap, vtx_fill, vx, vy, vz;

  const Int_t maxtracks = 1000; //I do not expect a larger number of tracks than this
  Int_t nseg[maxtracks], incoming[maxtracks], trk_num_holes[maxtracks], trk_max_gap[maxtracks];
  Float_t TX[maxtracks], TY[maxtracks], IP[maxtracks],  trackfill[maxtracks];; 


  //ana->SetBranchAddress("vtx_good",&vtx_good); // solo MC
  //ana->SetBranchAddress("mp_pdgID",&mp_pdgID); // solo MC
  //ana->SetBranchAddress("mp_motherID",&mp_motherID); // solo MC
  vtx->SetBranchAddress("vID",&vID);
  vtx->SetBranchAddress("vx",&vx);
  vtx->SetBranchAddress("vy",&vy);
  vtx->SetBranchAddress("vz",&vz);
  vtx->SetBranchAddress("vtx_max_aperture",&vtx_max_aperture);
  ...
  vtx->SetBranchAddress("trackfill",&trackfill);
  vtx->SetBranchAddress("trk_num_holes",&trk_num_holes);
  vtx->SetBranchAddress("trk_max_gap",&trk_max_gap);
 
  //outputtree to save
  TFile * outputfile = new TFile("tmva_evaluation_first_quarter_16_04_19.root","RECREATE");  
  ...
  outputvtx->Branch("vtx_max_aperture",&vtx_max_aperture,"vtx_max_aperture/F");
  ...
  outputvtx->Branch("vtx_node",&vtx_node,"vtx_node/F");
  outputvtx->Branch("vtx_holes",&vtx_holes,"vtx_holes/F");
  outputvtx->Branch("vtx_gap",&vtx_gap,"vtx_gap/F");
  ...
  for (int ivtx = 0; ivtx < nvertices; ivtx++){
	...
	vtx_node += incoming[itrk];
	vtx_holes += (trk_num_holes[itrk]*1.0)/nseg[itrk];
	vtx_gap += trk_max_gap[itrk];
	vtx_fill += trackfill[itrk];
	//std::cout << incoming[itrk] << " " << trk_num_holes[itrk] << " " << trk_max_gap[itrk] << " " << nseg[itrk] << " " << vtx_holes << std::endl;	
      }
      ...
      vtx_fill = (Float_t)(vtx_fill/ntracks);
      vtx_node = (Float_t)(vtx_node/ntracks);
      vtx_holes = (Float_t)(vtx_holes/ntracks);
      vtx_gap = (Float_t)(vtx_gap/ntracks);
      //std::cout << ivtx << " " << vtx_node << " " << vtx_holes << " " << vtx_gap << std::endl;
      outputvtx->Fill();
  }
  f->Close();
  outputfile->Write();
...
void prepareTMVA_evaluation_MC_tree(){
  TFile *f = TFile::Open("vtx_MC_16_04_19.root");
  //TFile *f = TFile::Open("MC_vtx_for_training.root");
  TTree *vtx = dynamic_cast<TTree*>(f->Get("vtx"));
  TTree *ana = dynamic_cast<TTree*>(f->Get("ana"));  // solo MC
  ...
  //outputtree to save
  TFile * outputfile = new TFile("tmva_evaluation_MC_15_04_19.root","RECREATE");  
  TTree *outputvtx = new TTree("vertices","Input file for TMVA analysis");
...
void prepareTMVAtree(){
  ...
  TTree *vtx = dynamic_cast<TTree*>(f->Get("vtx"));
  TTree *mck = dynamic_cast<TTree*>(f->Get("ana"));

  Char_t vtx_good;
  Int_t vID, mp_pdgID, ntracks;
  Float_t probability, maximpactparameter, meanTX, meanTY,meannseg, vtx_max_aperture;
  ...
  mck->SetBranchAddress("vtx_good",&vtx_good);
  mck->SetBranchAddress("mp_pdgID",&mp_pdgID);
  vtx->SetBranchAddress("vID",&vID);
  vtx->SetBranchAddress("vtx_max_aperture",&vtx_max_aperture);
  vtx->SetBranchAddress("n",&ntracks); 
  vtx->SetBranchAddress("probability",&probability);
...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TFile *input(0);
   TString fname = "tmva_input_vertices.root";
  ...
  //   TFile f("tmva_input_vertices.root");
   ...
   TTree *signalTree = sampleTree->CopyTree("vtx_good==1 && mp_pdgID!=22");
   TTree *backgroundTree = sampleTree->CopyTree("!(vtx_good==1 && mp_pdgID!=22)");
   ...
   */
   ...
   dataloader->AddVariable("vtx_max_aperture", 'F');
   dataloader->AddVariable("meanIP",'F');
   //dataloader->AddVariable("maxIP",'F');
   //dataloader->AddVariable("IP_var := meanIP/(maxIP-meanIP)",'F');
   //dataloader->AddVariable("hole_var:= (vtx_holes-vtx_gap)",'F');
   //dataloader->AddVariable("vtx_holes",'F');
   //dataloader->AddVariable("vtx_gap",'F');
   ...
   dataloader->PrepareTrainingAndTestTree( mycuts, mycutb,
                                        "nTrain_Signal=1000:nTrain_Background=2000:SplitMode=Random:NormMode=NumEvents:!V" );
