...
void TMVAClassificationApplication( int choice=-1, TString myMethodList = "" )
{
   ...
   Float_t meanIP, maxIP, IP_var;
   Float_t meanTX, meanTY, Fill_var, vx, vy, vz;
   Int_t vID;
   Float_t ntracks;
   reader->AddVariable("probability", &probability);
   reader->AddVariable("maxaperture", &maxaperture);  // tutto
   // reader->AddVariable("IP_var := meanIP/maxIP",&IP_var);
   reader->AddVariable("Fill_var := vtx_fill/ntracks",&Fill_var);
   //reader->AddVariable("meannseg",&meannseg);  // escluso primari
   //reader->AddVariable("ntracks",&ntracks);
   //reader->AddVariable("vtx_fill",&vtx_fill);
   reader->AddVariable("maxIP",&maxIP);
