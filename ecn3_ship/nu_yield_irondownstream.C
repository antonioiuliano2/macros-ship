...
void generate_neutrinos(int neutrinosource = 2, bool uselogbins=false){ //generate neutrino produced spectra according to Thomas histograms
 ...
 Float_t deltaz = 2650.; //Used in AdvSND (1.5 m after upstream target)
 Double_t targetdx = 30.; //for geometrical acceptance requirement
 Double_t targetdy = 30.;
...
void nu_yield_irondownstream(int neutrinosource = 2, bool uselogbins=false){ //passing neutrino types and interaction mode to nu_yield_general for estimation
...
Double_t nu_yield_general(int neutrinosource, bool uselogbins, const char* nu = "nu_mu", const char* intmode = "dis_cc", const char* charmmode = ""){     
  ...
  TGraph *hxsec_p = (TGraph*) xsec->Get(Form("%s_Fe56/%s_p%s",nu,intmode,charmmode));   
  TGraph *hxsec_n = (TGraph*) xsec->Get(Form("%s_Fe56/%s_n%s",nu,intmode,charmmode));
  TGraph *hxsec_other = (TGraph*) xsec->Get(Form("%s_Fe56/%s%s",nu,intmode,charmmode));
  ...
  const Int_t A = 56; //magnetized iron
  const Int_t Z = 26;

  Float_t Ninteracting = 0.;
  Double_t mass = 4986*1e+3; //mass in grams (AdvSND Downstream Target)
  //Double_t mass = 3080*1e+3; //mass in grams (ship ECN3 proposal)
  Double_t surface = 60. * 60.; //surface in square centimetres (squared configuration)
