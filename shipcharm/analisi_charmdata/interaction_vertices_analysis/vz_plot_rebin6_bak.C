...
*/
  ...
  for(int i=0;i<30;i++){
    ...
    if(i==2 || i ==3) out << i << " " << xgraph[i] << " " << yMC[i] << " " << err_MC[i] << " " << yDT[i] << " " << 4*err_DT[i] << endl;
    else if (i<10 && i%2==0) 
     out << i << " " << xgraph[i] << " " << yMC[i] + yMC[i+1] << " " << TMath::Sqrt(err_MC[i]*err_MC[i]+err_MC[i+1]*err_MC[i+1])
      << " " << yDT[i] + yDT[i+1]<< " " << TMath::Sqrt(err_DT[i]*err_DT[i]+err_DT[i+1]*err_DT[i+1]) << endl;
    else if (i<10 && i%2==1) 
     out << i << " " << xgraph[i] << " " << yMC[i] + yMC[i-1] << " " << TMath::Sqrt(err_MC[i]*err_MC[i]+err_MC[i-1]*err_MC[i-1])
      << " " << yDT[i] + yDT[i-1]<< " " << TMath::Sqrt(err_DT[i]*err_DT[i]+err_DT[i-1]*err_DT[i-1]) << endl;
    else out << i << " " << xgraph[i] << " " << yMC[i] << " " << err_MC[i] << " " << yDT[i] << " " << 4*err_DT[i] << endl;
  }
  ...
  for(int i=0;i<30;i++){
   ...
   for(int i=0;i<30;i++){
     ...
     if(i>2 && i<10 && i%2==0) grMChd->SetPointError(i/2+1,0,TMath::Sqrt(err_MC_hd[i]*err_MC_hd[i]+err_MC_hd[i+1]*err_MC_hd[i+1]));//points merged
   ...
   for(int i=0;i<30;i++){
     ...
     if(i>2 && i<10 && i%2==0) grMC->SetPointError(i/2+1,0,TMath::Sqrt(err_MC[i]*err_MC[i]+err_MC[i+1]*err_MC[i+1]));//points merged     
  ...
  for(int i=0;i<30;i++){
    ...
    if(i>2 && i<10 && i%2==0) grwb->SetPointError(i/2+1,0,TMath::Sqrt(err_DT[i]*err_DT[i]+err_DT[i+1]*err_DT[i+1]));//points merged
