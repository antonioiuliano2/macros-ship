...
path =  '/eos/experiment/ship/simulation/bkg/Mbias2025/CERN_combined_weighted/34/'
...
def processFile(fin,noCharm=True):
    ...
    for n in range(sTree.GetEntries()):
      ...
      for v in sTree.PlaneHAPoint:
       ...
       l10ptot = ROOT.TMath.Min(ROOT.TMath.Max(ROOT.TMath.Log10(p),-0.3),1.69999)
       l10pt   = ROOT.TMath.Min(ROOT.TMath.Max(ROOT.TMath.Log10(pt),-2.),0.4999)
       key = idhnu
       h[key].Fill(p,weight)
       key = idhnu+100
...
def run():
 ...
 ut.writeHists(h,'pythia8_Geant4_10.0_c0-1000_nu.root')
